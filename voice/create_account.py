...
def fit_gmm(username, means, invstds):
    ...
    try:
        userfeatures = pickle.loads(open(path + userfile, 'rb+').read(),encoding='latin1')
    except IOError:
...
def write_features(username):
    ...
    try:
        allfeatures = pickle.loads(open(filename, 'rb+').read(),encoding='latin1')
        os.remove(filename)
    except IOError:
        print("IO Error")
        allfeatures = {}
...
def recalculate_norm():
    ...
    try:
        allfeatures = pickle.load(open(filename, 'rb+'),encoding='latin1')
    except IOError:
